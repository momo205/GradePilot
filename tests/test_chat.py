...
def test_chat_session_materials_first_welcome(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    ...
    body = loaded.json()
    # Single-class wizard uses numeric phases 1–4 (see onboarding_gate).
...
def test_chat_class_then_syllabus_phase_no_timeline_until_phase3(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    user_id = str(uuid.uuid4())
    _auth(monkeypatch, user_id=user_id)
    _override_db(monkeypatch)
    ...
    low1 = r1.json()["messages"][-1]["content"].lower()
    assert "syllabus" in low1 or "upload" in low1

    # Timeline in phase 2 is ignored (syllabus upload comes first via separate API).
    r2 = client.post(
        f"/chat/sessions/{session_id}/messages",
        headers={"Authorization": "Bearer test"},
    ...
    assert r2.json()["state"].get("phase") == 2
